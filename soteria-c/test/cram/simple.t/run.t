...
Symbolic execution of a simple program with concrete values only
  ...
  Symex terminated with the following outcomes:
    [Ok: (2, { heap = [(V|1|, Freed); (V|2|, Freed)]; globs = [] })]
...
Symbolic execution of a simple program with symbolic values
  ...
  Symex terminated with the following outcomes:
    [Ok: (1, { heap = [(V|1|, Freed); (V|3|, Freed)]; globs = [] });
     Ok: (2, { heap = [(V|1|, Freed); (V|3|, Freed)]; globs = [] })]
...
Checking that code cannot branch infinitely
  ...
  Symex terminated with the following outcomes:
    [Ok: (0,
          { heap =
            [(V|1|, Freed); (V|2|, [Uninit {offset = 0; len = 4}; (Bound 4)]);
             (V|3|, Freed); (V|4|, [Uninit {offset = 0; len = 4}; (Bound 4)]);
             ...
             (V|17|, Freed);
             (V|18|, [Uninit {offset = 0; len = 4}; (Bound 4)])];
            globs = [] });
     Ok: (0,
          { heap =
            [(V|1|, Freed); (V|2|, [Uninit {offset = 0; len = 4}; (Bound 4)]);
  ...
  Symex terminated with the following outcomes:
    [Ok: (0,
          { heap = [(V|1|, Freed); (V|2|, Freed); (V|3|, Freed)]; globs = [] });
     Ok: (1, { heap = [(V|1|, Freed); (V|2|, Freed)]; globs = [] })]
...
Should return a single branch!
  ...
  Symex terminated with the following outcomes:
    [Ok: (b2i(((0 != V|4|) && (0 != V|2|))),
          { heap = [(V|1|, Freed); (V|3|, Freed)]; globs = [] })]
