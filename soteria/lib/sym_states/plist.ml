...
struct
  ...
  type syn = Ser_binding of (SInt.t * Elem.syn) | Ser_bound of SInt.t

  let no_fix = []
  let[@inline] lift_fix ~ofs (fix : Elem.syn) = Ser_binding (ofs, fix)
  ...
  let to_syn (m, b) : syn list =
    let bindings =
      ...
      |> Seq.concat_map (fun (k, v) ->
          Elem.to_syn v |> List.to_seq |> Seq.map (fun v -> Ser_binding (k, v)))
      |> List.of_seq
    in
  ...
  let pp_syn : Format.formatter -> syn -> unit =
   fun ft ser ->
    match ser with
    | Ser_binding (k, v) ->
        Fmt.pf ft "Binding: %a -> %a" SInt.pp k Elem.pp_syn v
