...
(** [join_results outcomes] partitions a list of results into successful values
    and errors. Returns [Ok oks] if all results are successful, or
    [Error errors] if any errors are present. *)
...
(** [combine3 l1 l2 l3] combines three lists into a list of triples. Raises
    [Invalid_argument] if the lists have different lengths. *)
...
(** [combine3_opt l1 l2 l3] combines three lists into a list of triples,
    returning [Some] result if the lists have equal length, or [None] if they
    differ. *)
...
(** [map2i f l1 l2] maps function [f] over two lists simultaneously, passing the
    current index as the first argument to [f]. The function [f] receives
    [(index, element1, element2)] for each pair of elements. Raises
    [Invalid_argument] if the lists have different lengths. *)
let map2i f l1 l2 = map2i 0 f l1 l2

(** [map_changed f l] maps function [f] over list [l] and returns both the
    resulting list and a boolean indicating whether any element was changed
...
(** [last l] returns the last element of list [l]. Raises [Invalid_argument] if
    the list is empty. *)
let rec last l =
  match l with [] -> invalid_arg "List.last" | [ x ] -> x | _ :: l -> last l

(** [last_opt l] returns [Some] of the last element of list [l], or [None] if
    the list is empty. *)
let rec last_opt l =
  match l with [] -> None | [ x ] -> Some x | _ :: l -> last_opt l

(** [concat_map2 f l1 l2] applies [f] to each pair of elements from [l1] and
    [l2], concatenating all resulting lists. Raises [Invalid_argument] if the
