...
module Make (V : Value_ext) = struct
  ...
  module rec Bool : Bool = struct
    ...
    and sem_eq v1 v2 =
      ...
      | ( BitVec n,
          ( Binop
              (Mul { checked = true }, { node = { kind = BitVec m; _ }; _ }, x)
          | Binop
              (Mul { checked = true }, x, { node = { kind = BitVec m; _ }; _ })
            ) )
      | ( ( Binop
              (Mul { checked = true }, { node = { kind = BitVec m; _ }; _ }, x)
          | Binop
  ...
  and BitVec : BitVec = struct
    ...
    let mask_cache : Z.t Array.t =
      Array.init 257 (fun n -> Z.(pred (one lsl n)))
    ...
    and mul ?(checked = false) v1 v2 =
      ...
      | ( ( Binop
              (Mul { checked = true }, x, { node = { kind = BitVec n; _ }; _ })
          | Binop
              (Mul { checked = true }, { node = { kind = BitVec n; _ }; _ }, x)
            ),
          BitVec m )
      | ( BitVec m,
          ( Binop
    ...
    and leq ~signed v1 v2 =
      Logs.L.debug (fun m -> m "leq: %a <= %a" pp v1 pp v2);
      assert (equal_ty v1.node.ty v2.node.ty);
      let bits = size_of v1.node.ty in
      match (v1.node.kind, v2.node.kind) with
