...
module Interval (Signed : sig
  ...
  module Range = struct
    ...
    let default_if_overflowed ((_, _, n) as r) =
      if overflowed r then default n else r
  ...
  include Soteria_std.Reversible.Make_mutable (struct
  ...
  let st_union =
    Var.Map.merge (fun _ l r ->
        match (l, r) with
        | l, None | None, l -> l
        | Some l, Some r -> Some (Range.union l r))
  ...
  let rec to_range n (v : Svalue.t) st : Range.t =
    ...
    | Nop (Add, vs) ->
        List.fold_left
          (fun acc v -> Range.map2 Z.add acc (to_range v))
          (Z.zero, Z.zero, n) vs
        |> Range.default_if_overflowed
    | Unop (Neg, v) ->
        let m, r, b = to_range v in
        Range.default_if_overflowed (Z.neg r, Z.neg m, b)
    | Binop (Rem false, _, r) ->
    ...
    | Ite (_, l, r) ->
        ...
        Range.union lr rr
    | Unop (BvExtend (false, _), v) -> to_range v
    | _ -> Range.default n
  ...
  let rec simplify (v : Svalue.t) st : Svalue.t =
    ...
    | Binop (AddOvf s, l, r) when s = Signed.signed ->
        let n = Svalue.size_of l.node.ty in
        let l = simplify l in
        let r = simplify r in
        let lr = to_range n l st in
        let rr = to_range n r st in
        let sum = Range.map2 Z.add lr rr in
        Svalue.Bool.bool @@ Range.overflowed sum
    | Binop (MulOvf s, l, r) when s = Signed.signed ->
        let n = Svalue.size_of l.node.ty in
        let l = simplify l in
        let r = simplify r in
        let lm, ln, _ = to_range n l st in
        let rm, rn, _ = to_range n r st in
        let corner, corners =
          (Z.mul lm rm, [ Z.mul lm rn; Z.mul ln rm; Z.mul ln rn ])
        in
        let min = List.fold_left Z.min corner corners in
        let max = List.fold_left Z.max corner corners in
        let prod = (min, max, n) in
        Svalue.Bool.bool @@ Range.overflowed prod
  ...
  let rec add_constraint ?(neg = false) ?(absorb = true) (v : Svalue.t) st :
    ...
    | Nop (And, vs) when not neg ->
        List.fold_left
          (fun ((v_acc, vars), st) v_and ->
            let (v_and', vars'), st' = add_constraint ~absorb v_and st in
            ((v_acc &&@ v_and', Var.Set.union vars vars'), st'))
          ((Svalue.Bool.v_true, Var.Set.empty), st)
          vs
    | Nop (Or, vs) when not neg ->
        let res =
          List.map (fun v_or -> add_constraint ~absorb:false v_or st) vs
        in
        (* we do the fold later, as we want the union of the sub-states *)
        List.fold_left
          (fun ((v_acc, vars), st) ((v_or', vars'), st') ->
            ((v_acc ||@ v_or', Var.Set.union vars vars'), st_union st st'))
          (List.hd res) (List.tl res)
