...
module Infix = struct
  ...
  let ( *!@ ) = ( *@ )
  let ( ~-! ) = ( ~- )
  let ( +?@ ) l r = (BitVec.add l r, BitVec.add_overflows ~signed:false l r)
  let ( +$?@ ) l r = (BitVec.add l r, BitVec.add_overflows ~signed:true l r)
  let ( -?@ ) l r = (BitVec.sub l r, BitVec.sub_overflows ~signed:false l r)
  let ( -$?@ ) l r = (BitVec.sub l r, BitVec.sub_overflows ~signed:true l r)
