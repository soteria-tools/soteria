module Graph = struct
  ...
  let tests =
    ...
    [
      test_case "empty reachable set" `Quick empty_reachable;
      test_case "one reachable set" `Quick one_reacheable;
      test_case "several reachable set 1" `Quick several_reachable_1;
      test_case "several reachable set 2" `Quick several_reachable_2;
      test_case "disconnected nodes" `Quick disconnected_nodes;
      test_case "all nodes reachable" `Quick all_reachable;
      test_case "single node with self-loop" `Quick single_self_loop;
      test_case "node with multiple edges" `Quick multiple_edges;
      test_case "topological order" `Quick topo_order;
    ]
end
...
module List = struct
  ...
  let tests = [ Alcotest.test_case "take count" `Quick take_count ]
end

let () =
  let open Alcotest in
  run "Soteria_std" [ ("Graph", Graph.tests); ("List", List.tests) ]
