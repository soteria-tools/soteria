  ...
  let simplify (x : t) =
    ...
    let* off = S_int.simplify x.off in
    let* addr = Symex.simplify x.addr in
    let* exposed = Symex.return x.exposed in
    Symex.return { off; addr; exposed }
  
  let sem_eq (a : t) (b : t) =
    Typed.Bool.and_
      (Typed.Bool.and_ (S_int.sem_eq a.off b.off)
         (Typed.sem_eq_untyped a.addr b.addr))
      (Typed.of_bool (a.exposed = b.exposed))
  Success ✅
