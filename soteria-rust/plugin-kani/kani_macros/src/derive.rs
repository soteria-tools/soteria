...
fn safety_conds_opt(
    ...
    if has_item_safety_constraint {
        Some(safe_body_from_struct_attr(
            &item_name,
            &derive_input,
            trait_name,
        ))
    } else if has_field_safety_constraints {
        Some(safe_body_from_fields_attr(
            &item_name,
            &derive_input.data,
            trait_name,
        ))
...
fn has_item_safety_constraint(ident: &Ident, derive_input: &DeriveInput, trait_name: &str) -> bool {
    let safety_constraints_in_item = derive_input
        .attrs
        .iter()
        .filter(|attr| attr.path().is_ident("safety_constraint"))
        .count();
...
fn has_field_safety_constraints_inner(_ident: &Ident, fields: &Fields) -> bool {
    match fields {
        Fields::Named(ref fields) => fields.named.iter().any(|field| {
            field
                .attrs
                .iter()
                .any(|attr| attr.path().is_ident("safety_constraint"))
        }),
    ...
    match fields {
        ...
        Fields::Named(ref fields) => {
            let safety_conds: Vec<TokenStream> = fields
                .named
                .iter()
                .filter_map(|field| parse_safety_expr(ident, field))
                .collect();
            quote! { #(#safety_conds)&&* }
        }
        Fields::Unnamed(_) => {
