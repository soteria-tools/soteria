...
rather than confusing the two errors in flight.
  ...
  bug: Terminated unwind in double_unwind::panic_in_drop_while_unwinding
      --> $TESTCASE_ROOT/double_unwind.rs:13:1
   10 |  fn panic_in_drop_while_unwinding() {
      |  ---------------------------------- 1: Entry point
      .  
   13 |  }
      |  ^ Triggering operation
  ...
  576 |                (*data).r = ManuallyDrop::new(f());
      |                                              --- 5: Call trace
      --> $TESTCASE_ROOT/double_unwind.rs:32:5
   28 |    fn nested_unwind_in_catch_unwind() {
      |    ---------------------------------- 1: Entry point
   29 |        let _ = std::panic::catch_unwind(|| {
      | /--------------'
   30 | |          let _d = CallingDrop;
   31 | |          panic!("first panic");
   32 | |      });
      | |      ^ Triggering operation
      | \-------' 2: Call trace
   33 |    }
...
Test exactly-evaluated float operations, at every precision
  ...
  • Steps: 567
  • Function calls: 257
  • Load accesses: 869 (92.87% through store)
  • Allocations: 50
