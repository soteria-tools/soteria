...
module Make (StateImpl : State.S) = struct
  ...
  *)
  ...
  and resolve_function ~in_tys ~out_ty :
    ...
    | FnOpRegular { kind = FunId (FBuiltin ZeroIfNull); generics = _ } ->
        perform_call (Synthetic ZeroIfNull)
    | FnOpRegular { kind = FunId (FBuiltin _); generics = _ } ->
        failwith "Builtin function calls shouldn't be generated by the frontend"
  ...
  and exec_fun (fn : Fun_kind.t) args =
    ...
    | Synthetic ZeroIfNull -> (
        match args with
        | [ Ptr (ptr, _) ] ->
            if%sat Sptr.is_null ptr then ok (Int Usize.(0s))
            else ok (Int Usize.(1s))
        | _ -> failwith "ZeroIfNull expects a single pointer argument")
    | Real fundef ->
