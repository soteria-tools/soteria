...
let t_ptr () = t_ptr (Crate.pointer_bits ())
let t_usize () = t_int (Crate.pointer_bits ())
...
module Ptr = struct
  ...
  let null () = null (Crate.pointer_bits ())
  let loc_of_int i = loc_of_int (Crate.pointer_bits ()) i
end
