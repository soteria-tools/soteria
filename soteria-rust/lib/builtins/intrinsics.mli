...
module M : (State : State_intf.S) -> sig
  ...
  (** Aborts the execution of the process.
      ...
      Note that, unlike most intrinsics, this is safe to call; it does not
      require an [unsafe] block. Therefore, implementations must not require the
      user to uphold any safety invariants.

      [std::process::abort](../../std/process/fn.abort.html) is to be preferred
      if possible, as its behavior is more user-friendly and more stable.

      The current implementation of [intrinsics::abort] is to invoke an invalid
      instruction, on most platforms. On Unix, the process will probably
      terminate with a signal like [SIGABRT], [SIGILL], [SIGTRAP], [SIGSEGV] or
  ...
  (** Calculates the offset from a pointer, potentially wrapping.
      ...
      resulting pointer to point into or at the end of an allocated object, and
      it wraps with two's complement arithmetic. The resulting value is not
      necessarily valid to be used to actually access memory.

      The stabilized version of this intrinsic is [pointer::wrapping_offset]. *)
  val arith_offset :
    t:Types.ty -> dst:rust_val -> offset:rust_val -> State.t -> ret

  (** A guard for unsafe functions that cannot ever be executed if [T] is
      uninhabited: This will statically either panic, or do nothing. It does not
      *guarantee* to ever panic, and should only be called if an assertion
      failure will imply language UB in the following code.

      This intrinsic does not have a stable counterpart. *)
  val assert_inhabited : t:Types.ty -> State.t -> ret

  (** A guard for [std::mem::uninitialized]. This will statically either panic,
      or do nothing. It does not *guarantee* to ever panic, and should only be
      called if an assertion failure will imply language UB in the following
      code.

      This intrinsic does not have a stable counterpart. *)
  val assert_mem_uninitialized_valid : t:Types.ty -> State.t -> ret

  (** A guard for unsafe functions that cannot ever be executed if [T] does not
      permit zero-initialization: This will statically either panic, or do
      nothing. It does not *guarantee* to ever panic, and should only be called
      if an assertion failure will imply language UB in the following code.

      This intrinsic does not have a stable counterpart. *)
  val assert_zero_valid : t:Types.ty -> State.t -> ret

  (** Informs the optimizer that a condition is always true. If the condition is
      false, the behavior is undefined.

      No code is generated for this intrinsic, but the optimizer will try to
      preserve it (and its condition) between passes, which may interfere with
      optimization of surrounding code and reduce performance. It should not be
      used if the invariant can be discovered by the optimizer on its own, or if
      it does not enable any significant optimizations.
  ...
  (** Reverses the bits in an integer type [T].
      ...
      The stabilized versions of this intrinsic are available on the integer
      primitives via the [reverse_bits] method. For example, [u32::reverse_bits]
  *)
  ...
  (** Reverses the bytes in an integer type [T].
      ...
      The stabilized versions of this intrinsic are available on the integer
      primitives via the [swap_bytes] method. For example, [u32::swap_bytes] *)
  ...
  (** Performs full-width multiplication and addition with a carry:
      ...
      This is possible without any overflow. For [uN]: MAX * MAX + MAX + MAX =>
      (2ⁿ-1) × (2ⁿ-1) + (2ⁿ-1) + (2ⁿ-1) => (2²ⁿ - 2ⁿ⁺¹ + 1) + (2ⁿ⁺¹ - 2) => 2²ⁿ
      \- 1

      For [iN], the upper bound is MIN * MIN + MAX + MAX => 2²ⁿ⁻² + 2ⁿ - 2, and
      the lower bound is MAX * MIN + MIN + MIN => -2²ⁿ⁻² - 2ⁿ + 2ⁿ⁺¹.

      This currently supports unsigned integers *only*, no signed ones. The
      stabilized versions of this intrinsic are available on integers. *)
  ...
  (** Rust's "try catch" construct for unwinding. Invokes the function pointer
      [try_fn] with the data pointer [data], and calls [catch_fn] if unwinding
      occurs while [try_fn] runs. Returns [1] if unwinding occurred and
      [catch_fn] was called; returns [0] otherwise.

      [catch_fn] must not unwind.

      The third argument is a function called if an unwind occurs (both Rust
      [panic] and foreign unwinds). This function takes the data pointer and a
      pointer to the target- and runtime-specific exception object that was
      caught.

      Note that in the case of a foreign unwinding operation, the exception
      object data may not be safely usable from Rust, and should not be directly
      exposed via the standard library. To prevent unsafe access, the library
      implementation may either abort the process or present an opaque error
      type to the user.

      For more information, see the compiler's source, as well as the
...
[valid]: crate::ptr#safety *)
  ...
  (** Deallocates a memory which allocated by [intrinsics::const_allocate] at
      compile time. At runtime, does nothing.
      ...
      - If the [ptr] is created in an another const, this intrinsic doesn't
        deallocate it.
      - If the [ptr] is pointing to a local variable, this intrinsic doesn't
        deallocate it. *)
...
otherwise, that principle should not be violated. *)
  ...
  (** Check if the post-condition [cond] has been met.
      ...
      By default, if [contract_checks] is enabled, this will panic with no
      unwind if the condition returns false.
  ...
  (** Returns whether we should perform contract-checking at runtime.
      ...
      This is meant to be similar to the ub_checks intrinsic, in terms of not
      prematurely committing at compile-time to whether contract checking is
      turned on, so that we can specify contracts in libstd and let an end user
      opt into turning them on. *)
  ...
  (** This is an accidentally-stable alias to [ptr::copy_nonoverlapping]; use
      that instead. *)
  ...
  (** Returns the number of leading unset bits (zeroes) in an integer type [T].
      ...
      let x = 0b0001_1100_u8; let num_leading = ctlz(x); assert_eq!(num_leading,
      3); []`
      ...
      let x = 0u16; let num_leading = ctlz(x); assert_eq!(num_leading, 16); []`
  *)
...
[]` *)
  ...
  (** Returns the number of bits set in an integer type [T]
      ...
      The stabilized versions of this intrinsic are available on the integer
      primitives via the [count_ones] method. For example, [u32::count_ones] *)
  ...
  (** Returns the number of trailing unset bits (zeroes) in an integer type [T].
      ...
      let x = 0b0011_1000_u8; let num_trailing = cttz(x);
      ...
      let x = 0u16; let num_trailing = cttz(x); assert_eq!(num_trailing, 16);
      []` *)
...
[]` *)
  ...
  (** Returns the value of the discriminant for the variant in 'v'; if [T] has
      no discriminant, returns [0].
  ...
  (** Combine two values which have no bits in common.
      ...
      This allows the backend to implement it as [a + b] *or* [a | b], depending
      which is easier to implement on a specific target.
  ...
  (** Float addition that allows optimizations based on algebraic rules.
      ...
      Stabilized as [f16::algebraic_add], [f32::algebraic_add],
      [f64::algebraic_add] and [f128::algebraic_add]. *)
  val fadd_algebraic : t:Types.ty -> a:rust_val -> b:rust_val -> State.t -> ret

  (** Float addition that allows optimizations based on algebraic rules. May
      assume inputs are finite.
  ...
  (** Float division that allows optimizations based on algebraic rules.
      ...
      Stabilized as [f16::algebraic_div], [f32::algebraic_div],
      [f64::algebraic_div] and [f128::algebraic_div]. *)
  val fdiv_algebraic : t:Types.ty -> a:rust_val -> b:rust_val -> State.t -> ret

  (** Float division that allows optimizations based on algebraic rules. May
      assume inputs are finite.

      This intrinsic does not have a stable counterpart. *)
  val fdiv_fast : t:Types.ty -> a:rust_val -> b:rust_val -> State.t -> ret

  (** Converts with LLVM’s fptoui/fptosi, which may return undef for values out
      of range (<https://github.com/rust-lang/rust/issues/10184>)
  ...
  (** Float multiplication that allows optimizations based on algebraic rules.
      ...
      Stabilized as [f16::algebraic_mul], [f32::algebraic_mul],
      [f64::algebraic_mul] and [f128::algebraic_mul]. *)
  ...
  (** Returns [a * b + c] for [f128] values, non-deterministically executing
      ...
      add instructions. It is unspecified whether or not a fused operation is
      selected, and that may depend on optimization level and context, for
      example. *)
  ...
  (** Float remainder that allows optimizations based on algebraic rules.
      ...
      Stabilized as [f16::algebraic_rem], [f32::algebraic_rem],
      [f64::algebraic_rem] and [f128::algebraic_rem]. *)
  val frem_algebraic : t:Types.ty -> a:rust_val -> b:rust_val -> State.t -> ret

  (** Float remainder that allows optimizations based on algebraic rules. May
      assume inputs are finite.
  ...
  (** Float subtraction that allows optimizations based on algebraic rules.
      ...
      Stabilized as [f16::algebraic_sub], [f32::algebraic_sub],
      [f64::algebraic_sub] and [f128::algebraic_sub]. *)
  val fsub_algebraic : t:Types.ty -> a:rust_val -> b:rust_val -> State.t -> ret

  (** Float subtraction that allows optimizations based on algebraic rules. May
      assume inputs are finite.
...
[]` *)
  ...
  (** Hints to the compiler that branch condition is likely to be true. Returns
  ...
  (** Returns [true] if the actual type given as [T] requires drop glue; returns
      [false] if the actual type provided for [T] implements [Copy].

      If the actual type neither requires drop glue nor implements [Copy], then
      ...
      The stabilized version of this intrinsic is
      [mem::needs_drop](crate::mem::needs_drop). *)
  val needs_drop : t:Types.ty -> State.t -> ret

  (** Emits a [nontemporal] store, which gives a hint to the CPU that the data
      should not be held in cache. Except for performance, this is fully
      equivalent to [ptr.write(val)].

      Not all architectures provide such an operation. For instance, x86 does
      not: while [MOVNT] exists, that operation is *not* equivalent to
      [ptr.write(val)] ([MOVNT] writes can be reordered in ways that are not
      allowed for regular writes). *)
  ...
  (** Calculates the offset from a pointer.
      ...
      This can only be used with [Ptr] as a raw pointer type ([*mut] or
      [*const]) to a [Sized] pointee and with [Delta] as [usize] or [isize]. Any
      other instantiations may arbitrarily misbehave, and that's *not* a
      compiler bug.

      # Safety

      If the computed offset is non-zero, then both the starting and resulting
      pointer must be either in bounds or at the end of an allocation. If either
      pointer is out of bounds or arithmetic overflow occurs then this operation
  ...
  (** The [prefetch] intrinsic is a hint to the code generator to insert a
      prefetch instruction if supported; otherwise, it is a no-op. Prefetches
      have no effect on the behavior of the program but can change its
      performance characteristics.

      The [locality] argument must be a constant integer and is a temporal
      locality specifier ranging from (0) - no locality, to (3) - extremely
      local keep in cache.
  ...
  (** See documentation of [<*const T>::guaranteed_eq] for details. Returns [2]
      if the result is unknown. Returns [1] if the pointers are guaranteed
      equal. Returns [0] if the pointers are guaranteed inequal. *)
  ...
  (** Determines whether the raw bytes of the two values are equal.
      ...
      Since this works by comparing the underlying bytes, the actual [T] is not
      particularly important. It will be used for its size and alignment, but
      any validity restrictions will be ignored, not enforced.

      # Safety

      It's UB to call this if any of the *bytes* in [*a] or [*b] are
      uninitialized. Note that this is a stricter criterion than just the
      *values* being fully-initialized: if [T] has padding, it's UB to call this
      intrinsic.

      At compile-time, it is furthermore UB to call this if any of the bytes in
      [*a] or [*b] have provenance.

      (The implementation is allowed to branch on the results of comparisons,
      which is UB if any of their inputs are [undef].) *)
  val raw_eq : t:Types.ty -> a:rust_val -> b:rust_val -> State.t -> ret

  (** This is an implementation detail of [crate::ptr::read] and should not be
  ...
  (** Performs rotate left.
      ...
      The stabilized versions of this intrinsic are available on the integer
      primitives via the [rotate_left] method. For example, [u32::rotate_left]
  *)
  ...
  (** Performs rotate right.
      ...
      The stabilized versions of this intrinsic are available on the integer
      primitives via the [rotate_right] method. For example, [u32::rotate_right]
  *)
  val rotate_right :
    t:Types.ty -> x:rust_val -> shift:rust_val -> State.t -> ret

  (** Returns the nearest integer to an [f128]. Rounds half-way cases to the
  ...
  *)
  ...
  (** Returns the nearest integer to an [f16]. Rounds half-way cases to the
      number with an even least significant digit.
  ...
  *)
  ...
  (** Returns the nearest integer to an [f32]. Rounds half-way cases to the
      number with an even least significant digit.
  ...
  *)
  ...
  (** Returns the nearest integer to an [f64]. Rounds half-way cases to the
      number with an even least significant digit.
  ...
  *)
  ...
  (** Returns the nearest integer to an [f128]. Rounds half-way cases away from
      zero.
  ...
  (** Returns the nearest integer to an [f16]. Rounds half-way cases away from
      zero.
  ...
  (** Returns the nearest integer to an [f32]. Rounds half-way cases away from
      zero.
  ...
  (** Returns the nearest integer to an [f64]. Rounds half-way cases away from
      zero.
  ...
  (** Magic intrinsic that derives its meaning from attributes attached to the
      function.

      For example, dataflow uses this to inject static assertions so that
      [rustc_peek(potentially_uninitialized)] would actually double-check that
      dataflow did indeed compute that it is uninitialized at that point in the
      control flow.
  ...
  (** The size of a type in bytes.
      ...
      More specifically, this is the offset in bytes between successive items of
      the same type, including alignment padding.
  ...
  (** Projects to the [index]-th element of [slice_ptr], as the same kind of
      pointer as the slice was provided -- so [&mut [T] → &mut T], [&[T] → &T],
      [*mut [T] → *mut T], or [*const [T] → *const T] -- without a bounds check.

      This is exposed via [<usize as SliceIndex>::get(_unchecked)(_mut)], and
      isn't intended to be used elsewhere.
      ...
      - [index < PtrMetadata(slice_ptr)], so the indexing is in-bounds for the
        slice
      - the resulting offsetting is in-bounds of the allocated object, which is
        always the case for references, but needs to be upheld manually for
        pointers *)
  ...
  (** Does a three-way comparison between the two arguments, which must be of
      character or integer (signed or unsigned) type.
...
[]` *)
  ...
  (** Like [transmute], but even less checked at compile-time: rather than
      ...
      **Undefined Behavior** at runtime.
      ...
      This is not expected to ever be exposed directly to users, rather it may
      eventually be exposed through some more-constrained API. *)
  ...
  (** Non-overlapping *typed* swap of a single value.
      ...
      # Safety Behavior is undefined if any of the following conditions are
      violated:
      ...
      * The region of memory beginning at [x] must *not* overlap with the region
      of memory beginning at [y].
  ...
  (** Returns whether we should perform some UB-checking at runtime. This
      eventually evaluates to [cfg!(ub_checks)], but behaves different from
      [cfg!] when mixing crates built with different flags: if the crate has UB
      checks enabled or carries the [#[rustc_preserve_ub_checks]] attribute,
      evaluation is delayed until monomorphization (or until the call gets
      inlined into a crate that does not delay evaluation further); otherwise it
      can happen any time.

      The common case here is a user program built with ub_checks linked against
      the distributed sysroot which is built without ub_checks but with
      [#[rustc_preserve_ub_checks]]. For code that gets monomorphized in the
      user crate (i.e., generic functions and functions with [#[inline]]),
      gating assertions on [ub_checks()] rather than [cfg!(ub_checks)] means
      that assertions are enabled whenever the *user crate* has UB checks
      enabled. However, if the user has UB checks disabled, the checks will
      still get optimized out. This intrinsic is primarily used by
      [crate::ub_checks::assert_unsafe_precondition]. *)
  val ub_checks : State.t -> ret

  (** Performs a volatile load from the [src] pointer The pointer is not
      required to be aligned.

      This intrinsic does not have a stable counterpart. *)
  val unaligned_volatile_load : t:Types.ty -> src:rust_val -> State.t -> ret

  (** Performs a volatile store to the [dst] pointer. The pointer is not
      required to be aligned.
  ...
  (** Returns the result of an unchecked addition, resulting in undefined
      behavior when [x + y > T::MAX] or [x + y < T::MIN].
  ...
  (** Performs an unchecked division, resulting in undefined behavior where
      ...
      Safe wrappers for this intrinsic are available on the integer primitives
      via the [checked_div] method. For example, [u32::checked_div] *)
  val unchecked_div : t:Types.ty -> x:rust_val -> y:rust_val -> State.t -> ret

  (** Returns the result of an unchecked multiplication, resulting in undefined
      behavior when [x * y > T::MAX] or [x * y < T::MIN].
  ...
  (** Returns the remainder of an unchecked division, resulting in undefined
      behavior when [y == 0] or [x == T::MIN && y == -1]

      Safe wrappers for this intrinsic are available on the integer primitives
      via the [checked_rem] method. For example, [u32::checked_rem] *)
  ...
  (** Performs an unchecked left shift, resulting in undefined behavior when
      ...
      Safe wrappers for this intrinsic are available on the integer primitives
      via the [checked_shl] method. For example, [u32::checked_shl] *)
  val unchecked_shl :
  ...
  (** Performs an unchecked right shift, resulting in undefined behavior when
      ...
      Safe wrappers for this intrinsic are available on the integer primitives
      via the [checked_shr] method. For example, [u32::checked_shr] *)
  val unchecked_shr :
    t:Types.ty -> u:Types.ty -> x:rust_val -> y:rust_val -> State.t -> ret

  (** Returns the result of an unchecked subtraction, resulting in undefined
      behavior when [x - y > T::MAX] or [x - y < T::MIN].
  ...
  (** Hints to the compiler that branch condition is likely to be false. Returns
      the value passed to it.
  ...
  (** Returns the number of variants of the type [T] cast to a [usize]; if [T]
      has no variants, returns [0]. Uninhabited variants will be counted.
      ...
      [crate::mem::variant_count]. *)
  val variant_count : t:Types.ty -> State.t -> ret

  (** Equivalent to the appropriate [llvm.memmove.p0i8.0i8.*] intrinsic, with a
  ...
  (** Equivalent to the appropriate [llvm.memcpy.p0i8.0i8.*] intrinsic, with a
      size of [count] * [size_of::<T>()] and an alignment of [align_of::<T>()].

      This intrinsic does not have a stable counterpart. # Safety

      The safety requirements are consistent with [copy_nonoverlapping] while
      the read and write behaviors are volatile, which means it will not be
      optimized out unless [_count] or [size_of::<T>()] is equal to zero.
  ...
  (** Equivalent to the appropriate [llvm.memset.p0i8.*] intrinsic, with a size
      of [count * size_of::<T>()] and an alignment of [align_of::<T>()].

      This intrinsic does not have a stable counterpart. # Safety

      The safety requirements are consistent with [write_bytes] while the write
      behavior is volatile, which means it will not be optimized out unless
      [_count] or [size_of::<T>()] is equal to zero.
  ...
  *)
  ...
  (** Returns (a + b) mod 2<sup>N</sup>, where N is the width of T in bits.
      ...
      The stabilized versions of this intrinsic are available on the integer
      primitives via the [wrapping_add] method. For example, [u32::wrapping_add]
  *)
  ...
  (** Returns (a * b) mod 2<sup>N</sup>, where N is the width of T in bits.
      ...
      The stabilized versions of this intrinsic are available on the integer
      primitives via the [wrapping_mul] method. For example, [u32::wrapping_mul]
  *)
  ...
  (** Returns (a - b) mod 2<sup>N</sup>, where N is the width of T in bits.
      ...
      The stabilized versions of this intrinsic are available on the integer
      primitives via the [wrapping_sub] method. For example, [u32::wrapping_sub]
  *)
  val wrapping_sub : t:Types.ty -> a:rust_val -> b:rust_val -> State.t -> ret

  (** This is an accidentally-stable alias to [ptr::write_bytes]; use that
      instead. *)
  val write_bytes :
    t:Types.ty ->
    dst:rust_val ->
  ...
  (** This is an implementation detail of [crate::ptr::write] and should not be
      used anywhere else. See its comments for why this exists.
