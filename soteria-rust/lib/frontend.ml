...
module Cmd = struct
  ...
  type t = {
    charon : string list; [@default []]
        (** Arguments passed to Charon (only when using [Charon] as a frontend)
        *)
    obol : string list; [@default []]
        (** Arguments passed to Obol (only when using [Obol] as a frontend) *)
  ...
  let build_cmd ~mode
    ...
    in
    let args = args @ List.concat_map entry_as_flag entry_points in
    match mode with
    | Rustc ->
...
let default () =
  ...
  Cmd.make
    ...
    ~obol:opaque_names
    ~entry_points:[ Attrib "rusteriatool::test"; Name "main" ]
    ~features:[ "rusteria" ]
    ~rustc:
      [
...
let merge_commands (cmds : Cmd.t list) =
  ...
  in
  let filter_entry_point crate (decl : fun_decl) =
    let ( let*! ) b f = if b then f () else None in
    let*! () =
      match (Config.get ()).filter with
      | [] -> true
      | filters ->
          ...
          List.exists
            ...
            filters
    in
    let*! () = List.exists (Cmd.entry_matches_fn decl) base_cmd.entry_points in
    let expect_error =
...
let parse_ullbc path =
  let mk_cmd, _ = create_using_current_config () in
  parse_ullbc_raw ~mk_cmd path

(** Given a path, will check if it has a [.rs] extension, in which case it will
    parse the ULLBC of that single file using rustc; otherwise will assume it's
    a path to a crate and use cargo. *)
let parse_ullbc_and_entry_points path =
  let mk_cmd, filter = create_using_current_config () in
  parse_ullbc_raw ~mk_cmd path |> with_entry_points ~filter
