...
module Ptr_tag_map = Ptr_tag.WeakMap
module Ptr_tag_set = Ptr_tag.WeakSet
...
let pp ft { tags; _ } =
  Fmt.iter_bindings Ptr_tag_map.iter
...
let init ?(initial_state = Unique) () =
  ...
  let parents = Ptr_tag_set.singleton tag in
  let node = { protector = None; parents; initial_state } in
  let tags = Ptr_tag_map.singleton tag node in
  (tag, { tags; known_size = 1; next_compact_at = compact_threshold })

let ub_state = fst @@ init ~initial_state:Disabled ()

(* [Gc.major] only finishes the current major cycle, so it may miss nodes. A
   second [Gc.major] completes that fresh cycle, making a double call equivalent
   to [Gc.full_major] whilst only paying for it when just [Gc.major] isn't
   enough. *)
...
let[@inline] compact ({ tags; known_size; next_compact_at } as st : t) =
  if known_size < next_compact_at then st
    ...
    let tags, known_size = collect tags in
    let tags, known_size =
      if known_size < next_compact_at then (tags, known_size) else collect tags
    in
    ...
    let next_compact_at =
      ...
      else Int.max compact_threshold (known_size * 2)
    in
...
let borrow ?protector parent ~state (st : t) =
  ...
  let node_parent = Ptr_tag_map.find parent tags in
  let parents = Ptr_tag_set.copy_and_add node_parent.parents tag in
  let node = { protector; parents; initial_state = state } in
  let tags = Ptr_tag_map.add tag node tags in
  (tag, { st with tags; known_size = known_size + 1 })

let unprotect tag ({ tags; _ } as st) =
  let tags =
    Ptr_tag_map.update tag
      (function
...
let set_protector ~protected tag ({ tags; _ } : t) =
  Ptr_tag_map.update tag (function
    | None ->
        let node = Ptr_tag_map.find tag tags in
