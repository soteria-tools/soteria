...
(** [match%ty x with ...] matches on the {e runtime} type of the typed value [x]
    ...
    boilerplate (and the matching footgun of casting to the wrong type).

    Several scrutinees are also supported; only the {e first} is the [get_ty]
    subject (and gets refined), the rest are matched as-is:
    {[
...
(* The phantom-type table: runtime-type constructor -> phantom tag key. The keys
   index into [Typed.T]; ["seq"] is special-cased (it is the applied type [T.any
   T.sseq]). *)
...
let key_of_ext_ctor = function
  ...
  | "TAdt" | "TTuple" | "TPolyType" -> Some "adt"
  | _ -> None
...
(* Classifies a [ty] pattern to the phantom tag key it refines the scrutinee to,
   ...
   or-patterns whose branches refine to different types. *)
let rec classify pat =
  ...
  | Ppat_construct (lid, arg) -> (
      ...
      | "TExtension" -> (
          match arg with Some (_, p) -> classify_ext p | None -> None)
      | ctor -> key_of_ty_ctor ctor)
  | _ -> None

and classify_ext pat =
  match pat.ppat_desc with
  | Ppat_constraint (p, _) | Ppat_alias (p, _) -> classify_ext p
  | Ppat_construct (lid, _) -> key_of_ext_ctor (Longident.last_exn lid.txt)
  | _ -> None
