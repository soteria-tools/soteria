...
Soteria PHP IR schema version 10.
...
| Class declarations | Partially supported | Ordinary named classes containing public, untyped, non-static properties and public, untyped, non-static instance methods are supported. Property defaults may use supported scalar literals, nested array literals, and numeric unary signs. Inheritance, interfaces, traits, attributes, property hooks, typed properties, and other class, property, or method modifiers remain unsupported. |
| `new` | Partially supported | Statically named supported user classes and built-in throwable classes may be constructed. Declared property initialization precedes a supported `__construct` call, whose required arguments, `$this` binding, mutation, and thrown control are modelled. Arguments to classes without a constructor are evaluated and ignored as in PHP. Throwable constructors accept no arguments or one concrete or coercible message argument. Dynamic class names remain unsupported. |
| Instance method calls | Partially supported | Statically named public instance methods use case-insensitive lookup on the receiver's runtime class. The receiver is checked before arguments are evaluated, calls use fresh local scopes containing `$this` and parameters, and returns, recursion, runtime errors, and throws are supported. Dynamic method names, named or reference arguments, unpacking, and non-public or static methods remain unsupported. |
| Object identity and properties | Partially supported | Each construction creates a stable object handle and a persistent property store. Assignment copies the handle, so aliases share updates while separate objects remain independent. Declared property reads, writes, references, `unset`, nested array access, method mutation, and branch isolation are supported. Static-name dynamic-property writes emit PHP's deprecation event; dynamic property names, cloning, and serialization remain unsupported. |
