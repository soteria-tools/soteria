...
Soteria PHP IR schema version 11.
...
| Class declarations | Partially supported | Ordinary named classes containing public, protected, or private untyped non-static properties and instance methods are supported. Property defaults may use supported scalar literals, nested array literals, and numeric unary signs. Inheritance, interfaces, traits, attributes, property hooks, typed properties, and static, abstract, final, or readonly members remain unsupported. |
| `new` | Partially supported | Statically named supported user classes and built-in throwable classes may be constructed. Declared property initialization precedes a supported `__construct` call, whose visibility, required arguments, `$this` binding, mutation, and thrown control are modelled. Inaccessible constructor arguments are not evaluated. Arguments to classes without a constructor are evaluated and ignored as in PHP. Throwable constructors accept no arguments or one concrete or coercible message argument. Dynamic class names remain unsupported. |
| Instance method calls | Partially supported | Statically named public, protected, and private instance methods use case-insensitive lookup on the receiver's runtime class. The receiver and visibility are checked before arguments are evaluated. Calls use fresh local scopes containing `$this` and parameters, preserve the declaring-class context, and support returns, recursion, runtime errors, and throws. Protected access through subclasses awaits inheritance support. Dynamic method names, named or reference arguments, unpacking, and static methods remain unsupported. |
| Object identity and properties | Partially supported | Each construction creates a stable object handle and a persistent property store. Declared cells are keyed by declaring class and source name. Assignment copies the handle, so aliases share updates while separate objects remain independent. Visibility is enforced with catchable `Error` values for reads, writes, references, `unset`, and nested access. Property-assignment right-hand sides run before an access error, matching PHP. Static-name dynamic-property writes emit PHP's deprecation event; dynamic property names, cloning, and serialization remain unsupported. |
