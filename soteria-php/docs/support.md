...
| Namespaces and imports | Partially supported | Namespace blocks, `use`, and group-use declarations are resolved by the frontend for supported declarations and class references. Namespace-relative dynamic names remain unsupported. |
| `include`, `include_once`, `require`, and `require_once` | Partially supported | Concrete leading top-level paths composed from strings, `__DIR__`, `__FILE__`, and concatenation are resolved deterministically. Included files may contain declarations, `declare(strict_types=1)`, imports, and further leading includes, but not executable statements. Dynamic paths, missing files, late includes, repeated non-`_once` includes, include return values, and runtime declaration activation are explicitly unsupported. |
| Composer autoloading | Partially supported | A leading `vendor/autoload.php` registration is recognized without executing project PHP. Root and installed-package Composer metadata provide PSR-4, classmap, and files autoloading. Statically referenced classes are loaded transitively; Composer files and loaded class units must fit the declaration-only include subset. PSR-0, custom autoloaders, exclusion rules, plugins, and autoload side effects remain unsupported. |
...
| Named functions and returns | Partially supported | Top-level declarations, required untyped by-value parameters, case-insensitive calls, early returns, fallthrough-to-`null`, recursion, first-class named-function callables, and the argument-free `#[Soteria\Test]` marker are supported. Extra arguments are evaluated and ignored as in PHP. Parameter defaults and types, return types, references, variadics, named arguments, other attributes, and nested declarations remain unsupported. |
| Analysis entry points | Partially supported | `exec --function NAME` selects a zero-argument named function or public `Class::method` case-insensitively and skips top-level executable statements. Instance methods receive a fresh normally constructed object. `discover FILE` lists functions and public methods marked with argument-free `#[Soteria\Test]` in deterministic declaration order. Entry parameters and attributes on interfaces, traits, or non-public methods are unsupported. |
...
| Class declarations | Partially supported | Ordinary named classes may extend one supported class and implement supported interfaces. Class and interface graphs are validated case-insensitively and cycles are rejected. Public, protected, and private untyped instance and static properties and methods are supported, including inherited layout, overriding, constructor inheritance, built-in throwable subclasses, and `#[Soteria\Test]` on supported public methods. Property defaults may use supported scalar literals, nested array literals, and numeric unary signs. Class/property attributes, other method attributes, property hooks, typed properties, and abstract, final, or readonly members remain unsupported. |
...
| Object builtins | Partially supported | `get_class`, `is_a`, `property_exists`, and `method_exists` support object values and concrete class/member strings. `is_a` supports its optional concrete `allow_string` flag and implicit `Stringable` conformance through `__toString`. Property and method queries do not invoke magic hooks or trigger additional Composer loads. Symbolic names remain unsupported. |
