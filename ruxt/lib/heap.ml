...
module StateKey = struct
  ...
  let fresh_rsym () =
    nondet (Typed.t_loc (8 * Charon_util.size_of_uint_ty Values.Usize))
...
module Block = struct
  ...
  let with_tree_block (f : 'a Tree_block.SM.t) : 'a SM.t =
  ...
  let with_tree_block_read_tb (f : Tree_borrow.t -> 'a Tree_block.SM.t) :
      'a SM.t =
    let* t_opt = SM.get_state () in
    let tree, tb = of_opt t_opt in
  ...
  let produce a = with_tree_block (Tree_block.produce a)
end
...
type serialized_globals = serialized list * T.sloc Typed.t list
[@@deriving show { with_path = false }]

let serialize_globals globals =
  ListLabels.fold_left (GlobMap.bindings globals) ~init:[]
    ~f:(fun acc (_, ((ptr : Sptr.t), _)) -> Typed.Ptr.loc ptr.ptr :: acc)

let serialize st : serialized_globals =
  Option.fold st ~none:([], []) ~some:(fun st ->
      let heap =
        Option.fold st.heap ~none:[] ~some:(fun st -> Heap.serialize st)
      in
      let globals = serialize_globals st.globals in
      (heap, globals))

let subst_serialized (subst_var : Svalue.Var.t -> Svalue.Var.t)
    ((heap, globals) : serialized_globals) : serialized_globals =
  let heap = List.map (Heap.subst_serialized subst_var) heap in
  let globals = List.map (Typed.subst subst_var) globals in
  (heap, globals)

let iter_vars_serialized ((heap, globals) : serialized_globals)
    (f : Svalue.Var.t * [< Typed.T.cval ] Typed.ty -> unit) : unit =
  List.iter (fun ser -> Heap.iter_vars_serialized ser f) heap;
  List.iter (fun ser -> Typed.iter_vars ser f) globals
...
let with_heap (f : 'a Heap.SM.t) : 'a SM.t =
  let* st = SM.get_state () in
  let st = of_opt st in
...
let produce (heap : serialized) : unit SM.t = with_heap (Heap.produce heap)

let consume (_heap : serialized) :
    (unit, [> Rustsymex.lfail ], serialized) SM.Result.t =
