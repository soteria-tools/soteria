...
let with_ptr_raw (ptr : Sptr.t) (st : block SPmap.t option) globals
    (f :
...
let with_ptr ptr st f =
  let globals = st.globals in
  with_state st (fun st -> with_ptr_raw ptr st globals f)
...
let apply_parser (type a) ?(ignore_borrow = false) ptr
  ...
  let handler (ty, ofs) st =
    let@ _, block = with_ptr_raw ptr st globals in
...
and fake_read ((_, meta) as ptr) ty st =
  let is_uncheckable_dst =
    ...
    | VTable _ ->
        ...
        true
  in
  if is_uncheckable_dst then return (None, st)
  else (
    L.debug (fun m ->
